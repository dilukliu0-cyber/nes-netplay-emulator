...
export class GbEmulatorSession implements EmulatorSession {
  ...
  private async launch(): Promise<void> {
    if (!this.canvas || !this.romBytes) {
      throw new Error("GB: session is not ready");
    }
    ...
    try {
      const emulator = await Nostalgist.gb({
        element: this.canvas,
        rom: {
          fileName: "game.gb",
          fileContent: romBlob
  ...
  private handleError(error: unknown): void {
    const err = error instanceof Error ? error : new Error("GB: emulator error");
    this.onError?.(err);
  }
}
