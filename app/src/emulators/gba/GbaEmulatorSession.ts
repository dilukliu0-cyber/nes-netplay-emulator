...
export class GbaEmulatorSession implements EmulatorSession {
  ...
  private async launch(): Promise<void> {
    if (!this.canvas || !this.romBytes) {
      throw new Error("GBA: session is not ready");
    }
    ...
    try {
      const emulator = await Nostalgist.gba({
        element: this.canvas,
        rom: {
          fileName: "game.gba",
          fileContent: romBlob
  ...
  private handleError(error: unknown): void {
    const err = error instanceof Error ? error : new Error("GBA: emulator error");
    this.onError?.(err);
  }
}
