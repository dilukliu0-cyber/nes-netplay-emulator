...
const appDisplayName = "nes netplay online 4.1";
...
async function ensureStreamFirewallAccess(): Promise<{ ok: boolean; message?: string }> {
  ...
  const consent = await dialog.showMessageBox({
    ...
    message: "Allow Windows firewall access for streaming mode?",
    detail: "We need to open inbound rules for this executable. Admin confirmation may be required.",
  ...
  if (!addTcp || !addUdp) {
    return {
      ok: false,
      message: "Failed to configure firewall rules. Streaming may be unavailable in this network."
    };
  }
